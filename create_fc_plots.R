...
# load limma data
leflu_data <- read.csv("limma_results_leflunomide_new.csv", as.is=TRUE)
flucon_data <- read.csv("limma_results_fluconazole_new.csv", as.is=TRUE)
ifos_data <- read.csv("limma_results_ifosfamide_new.csv", as.is=TRUE)
...
leflu_significant <- leflu_data[leflu_data$P.Value < 0.05,]
flucon_significant <- flucon_data[flucon_data$P.Value < 0.05,]
ifos_significant <- ifos_data[ifos_data$P.Value < 0.05,]
...
# save to a png
ggsave("logfc_hist_new.png", plot = all_hist, width=6.5, height=2.5, units="in")
...
leflu_highlight <- leflu_data[which(leflu_data$X %in% c("1370269_at", "1392946_at", "1373810_at")),]
flucon_highlight <- flucon_data[which(flucon_data$X %in% c("1377014_at", "1371076_at", "1390255_at")),]
ifos_highlight <- ifos_data[which(ifos_data$X %in% c("1370355_at")),]
...
# save to a png
ggsave("logfc_scat_volcano_notsig_new.png", plot = all_scat, width=6.5, height=2.5, units="in")